...
export default function KnowledgeGraph({ onTopicClick }) {
  ...
  const runForceSimulation = useCallback(() => {
    ...
    const step = () => {
      ...
      currentNodes.forEach(n => {
        ...
        if (!p) return
        forces[n.id].x += (cx - p.x) * 0.01; forces[n.id].y += (cy - p.y) * 0.01
      })
      energy = 0
      setPositions(prev => {
        const next = { ...prev }
  ...
  useEffect(() => {
    if (nodes.length > 0 && links.length > 0 && !simRunningRef.current) {
      simRunningRef.current = true
      runForceSimulation()
    }
  }, [links.length])
  ...
  const onMove = useCallback((e) => {
    if (dragId) {
      ...
      setPositions(prev => ({ ...prev, [dragId]: { x: ox + (e.clientX - mx) / transform.scale, y: oy + (e.clientY - my) / transform.scale } }))
    } else if (panning) {
  ...
  const onUp = useCallback(() => {
    ...
    setDragId(null); setPanning(false)
    if (wasDragging && !simRunningRef.current) { simRunningRef.current = true; runForceSimulation() }
  ...
  const handleNodeClick = useCallback(async (node) => {
    ...
    try {
      ...
      topics.forEach((sub, i) => {
        ...
        childPositions[childId] = { x: parentPos.x + childRadius * Math.cos(angle), y: parentPos.y + childRadius * Math.sin(angle) }
      })
  ...
  if (!nodes.length) {
    return (<div ref={containerRef} className="main-area" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div className="empty-state"><div className="empty-state-icon"> </div><div>导入笔记或新建知识，知识图谱将在这里展示</div></div></div>)
  }
  ...
  Object.entries(subTopics).forEach(([parentId, data]) => {
    ...
    ;(data.topics || []).forEach((sub, i) => {
      allItems.push({ id: `sub_${parentId}_${i}`, title: typeof sub === 'string' ? sub : sub.title || sub.name || String(sub), category: nodes.find(n => n.id === Number(parentId))?.category || 'Other', isMain: false, parentId: Number(parentId) })
    })
  })

  const arrows = links.map((lk, i) => {
    const s = positions[lk.source], t = positions[lk.target]
    if (!s || !t) return null
    const label = lk.source_topic && lk.target_topic ? `${lk.source_topic} ↔ ${lk.target_topic}` : null
  ...
  const subArrows = Object.entries(subTopics).filter(([parentId]) => nodeIds.has(Number(parentId))).map(([parentId, data]) => {
    const result = []
    ;(data.topics || []).forEach((sub, i) => {
      const s = positions[parentId], t = positions[`sub_${parentId}_${i}`]
      if (s && t) result.push(<Arrow key={`sub_link_${parentId}_${i}`} s={s} t={t} color="rgba(74,222,128,0.5)" dashed thin />)
    })
    ;(data.relations || []).forEach(([i, j], idx) => {
      const s = positions[`sub_${parentId}_${i}`], t = positions[`sub_${parentId}_${j}`]
      if (s && t) result.push(<Arrow key={`sub_rel_${parentId}_${idx}`} s={s} t={t} color="rgba(251,191,36,0.8)" bidirectional thin />)
    })
    return result
  })

  const topicLinks = links.filter(lk => lk.source_topic && lk.target_topic).map((lk, idx) => {
    const srcData = subTopics[lk.source], tgtData = subTopics[lk.target]
    if (!srcData || !tgtData) return null
    const srcIdx = (srcData.topics || []).indexOf(lk.source_topic), tgtIdx = (tgtData.topics || []).indexOf(lk.target_topic)
    if (srcIdx < 0 || tgtIdx < 0) return null
    const s = positions[`sub_${lk.source}_${srcIdx}`], t = positions[`sub_${lk.target}_${tgtIdx}`]
    if (!s || !t) return null
    return <Arrow key={`topic_link_${idx}`} s={s} t={t} color="rgba(251,191,36,0.9)" bidirectional thin />
  })

  return (
    <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden', cursor: panning ? 'grabbing' : 'grab' }} onMouseDown={onBgDown} onMouseMove={onMove} onMouseUp={onUp} onMouseLeave={onUp}>
      <div style={{ position: 'absolute', transform: `translate(${transform.x}px, ${transform.y}px) scale(${transform.scale})`, transformOrigin: '0 0', width: '100%', height: '100%' }}>
        <svg width="100%" height="100%" style={{ position: 'absolute', top: 0, left: 0, pointerEvents: 'none', overflow: 'visible' }}>{arrows}{subArrows}{topicLinks}</svg>
        {allItems.map(n => {
