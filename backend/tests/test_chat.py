...
class TestConversation:
    ...
    def test_get_conversation_with_messages(self, client):
        ...
        with patch("app.services.chat_service.generate_chat_response", return_value={"content": "AI 回复", "source_ids": [], "is_from_kb": False, "found_in_kb": False}):
            msg = client.post(
...
class TestSendMessage:
    ...
    def test_auto_title_on_first_message(self, client):
        ...
        with patch("app.services.chat_service.generate_chat_response", return_value={"content": "ok", "source_ids": [], "is_from_kb": False, "found_in_kb": False}):
            resp = client.post(
                f"/api/chat/conversations/{conv['id']}/messages",
