...
@router.post("/file")
async def import_single_file(file: UploadFile = File(...), background: BackgroundTasks = None,
    ...
    source = Source(filename=file.filename, file_type=ext, node_count=len(sections), user_id=current_user.id)
    db.add(source)
    db.flush()
    node_ids = []
    for title, text in sections:
        content_hash = hashlib.sha256(text.encode()).hexdigest()
        node = KnowledgeNode(title=title, content=text, source_id=source.id, content_hash=content_hash, user_id=current_user.id)
        db.add(node)
        db.flush()
        node_ids.append(node.id)
        if background:
            background.add_task(_safe_add_to_index, node.id, text, title, current_user.id)
    db.commit()
    return {"source_id": source.id, "node_count": len(node_ids), "node_ids": node_ids}


@router.post("/batch")
async def import_batch_files(files: list[UploadFile] = File(...), background: BackgroundTasks = None,
    db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
@router.post("/text")
async def import_text(body: dict, background: BackgroundTasks = None,
    db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    source = Source(filename=title, file_type="text", node_count=len(sections), user_id=current_user.id)
    db.add(source)
    db.flush()
    node_ids = []
    for section_title, text in sections:
        content_hash = hashlib.sha256(text.encode()).hexdigest()
        node = KnowledgeNode(title=section_title, content=text, source_id=source.id, content_hash=content_hash, user_id=current_user.id)
        db.add(node)
        db.flush()
        node_ids.append(node.id)
        if background:
            background.add_task(_safe_add_to_index, node.id, text, section_title, current_user.id)
    db.commit()
    return {"source_id": source.id, "node_count": len(node_ids), "node_ids": node_ids}
